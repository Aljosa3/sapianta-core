"""Replay validation for governed execution sessions."""
...
from .governed_session_validator import validate_governed_execution_session


def validate_session_replay(session: dict) -> dict:
    before = stable_hash(session)
    snapshot = deepcopy(session)
    validation = validate_governed_execution_session(session)
