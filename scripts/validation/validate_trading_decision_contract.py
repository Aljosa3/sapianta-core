#!/usr/bin/env python3
"""
Governance-level validation for TRADING_DECISION_CONTRACT_v0.1.json
...
def validate_contract_data(contract: Dict[str, Any]) -> None:
    ...
    if contract.get("contract_name") != "TRADING_DECISION_CONTRACT":
        raise ValueError("contract_name must be 'TRADING_DECISION_CONTRACT'.")

    if contract.get("contract_version") != "0.1":
        raise ValueError("contract_version must be '0.1'.")

    if contract.get("event_name") != "TRADING_DECISION_PROPOSED":
        raise ValueError("event_name must be 'TRADING_DECISION_PROPOSED'.")
    ...
    max_rationale_len = constraints.get("max_rationale_len")
    ...
    if not isinstance(max_rationale_len, int) or max_rationale_len <= 0:
        raise ValueError("constraints.max_rationale_len must be a positive integer.")

    # minimal required field presence expectation
    must_have = {"decision_id", "signal_id", "action", "symbol", "timeframe"}
...
def main(argv: List[str]) -> int:
    ...
    default_path = os.path.join(
        repo_root, "governance", "domains", "trading", "TRADING_DECISION_CONTRACT_v0.1.json"
    )
    ...
    try:
        ...
        return _fail(f"[TRADING_DECISION_CONTRACT_VALIDATE] FAIL: {e}")

    sys.stdout.write("[TRADING_DECISION_CONTRACT_VALIDATE] PASS\n")
    return 0
